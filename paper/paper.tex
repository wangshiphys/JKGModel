...
\section{Classical energy and moment direction of the stripe ordered states}
...
\section{Classical ground states for some special model parameters}

\subsection{$J=K=0, \Gamma=1$}
For pure antiferromagetic $\Gamma$ model, the classical ground state is highly degenerate including FM state as well as states with no long range order. The ordered moment of the FM state lie in a plane that passes the coordinate origin and is perpendicular to the $[111]$ direction. Fig.~\ref{fig:SpinConfig}(a) show a typical spin configuration of those disordered states that have the same energy as the FM state.

\subsection{$J=K=0, \Gamma=-1$}
For pure ferromagnetic $\Gamma$ model, we found several energetically degenerate states as the classcial ground state, including FM state, stripe states and a noncollinear state. The FM state is shown in Fig.~\ref{fig:SpinConfig}(b) and the ordered moment is along the $[111]$ direction. As for the stripe states, there are three degenerate spin configurations as shown in Fig.~\ref{fig:SpinConfig}(c)-(e) and the directions for the cyan, red, pink, green, yellow and blue arrows are $[\bar{1}11]$, $[1\bar{1}\bar{1}]$, $[1\bar{1}1]$, $[\bar{1}1\bar{1}]$, $[11\bar{1}]$ and $[\bar{1}\bar{1}1]$, respectively. Apart from these collinear states, a noncollinear state exist as the classical ground state. The magnetic unit-cell contains four lattice sites. The directions for the yellow, gray, pink, cyan arrows in Fig.~\ref{fig:SpinConfig}(f) are $[11\bar{1}]$, $[111]$, $[1\bar{1}1]$ and $[\bar{1}11]$, respectively.

\subsection{$J=\Gamma=0, K=1$}
The ground state for the classical antiferromagetic Kitaev model is also degenerate involving three nematic ordered states and three stripe ordered states. For the nematic ordered state shown in Fig.~\ref{fig:SpinConfig}(g), the spins on the chains along the $x$-bond direction form antiferromagetic chains and the direction for the orange arrow is $[100]$. There is no association between different chains. Similarly, the antiferromagetic chains along the $y$-bond and $z$-bond direction for Fig.~\ref{fig:SpinConfig}(h) and Fig.~\ref{fig:SpinConfig}(i) respectively. The direction for the orange arrow in Fig.~\ref{fig:SpinConfig}(h) is $[010]$ and in Fig.~\ref{fig:SpinConfig}(i) is $[001]$. In addition to these nematic ordered states, there are stripe ordered states that have the same energy as the nematic ordered states. They have the same pattern as that is shown in Fig.~\ref{fig:SpinConfig}(c)-(e), but with different ordered moment directions. For the stripe ordered states shown in Fig.~\ref{fig:SpinConfig}(c)-(e) the ordered moment lie in the $yz$, $xz$ and $xy$ plane, respectively.

\begin{figure*}
    \centering
    \includegraphics[width=0.96\textwidth]{FigA4.pdf}
    \caption{\label{fig:SpinConfig}(Color online) Classical ground state spin configurations for some special model parameters. For subfigure (a)-(f), the spin vectors were projected to the $xy$ plane and different spin vectors were represented by different colors. (a) Typical disordered spin configuration for $J=K=0,\Gamma=1$. (b)-(f) Spin configurations for $J=K=0,\Gamma=-1$. (g)-(i) Nematic ordered spin configurations for $J=\Gamma=0, K=1$.}
\end{figure*}
