...
PS_DATA_NAME_TEMP = "PS_num1={0}_num2={1}_which={2}_config={3}_alpha={4:.4f}_beta={5:.4f}.npz"
...
ps_data_name = PS_DATA_NAME_TEMP.format(num1, num2, which, config, alpha, beta)
...
probabilities = StripeDirection(
    thetas, phis, ket, num1=num1, num2=num2, which=which, config=config
)
np.savez(
    PS_DATA_PATH + ps_data_name,
    size=[num1, num2], which=[which], config=[config], parameters=[alpha, beta],
...
plt.show()
fig.savefig(PS_DATA_PATH + ps_fig_name, dpi=100, transparent=True)
plt.close("all")
